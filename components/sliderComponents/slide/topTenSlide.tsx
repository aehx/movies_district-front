...
export default function TopTenSlide(props: TMDBMovieData) {
  ...
  return (
    <div className="group overflow-hidden relative" onClick={() => dispatch(addTopTenTrailer(id))}>
      <div
        className="w-full overflow-hidden min-h-[23vh] relative">
        <Image src={`${img_base_url}${props.backdrop_path}`} alt={props.title} sizes="(max-width: 768px) 100vw,(max-width: 1200px) 50vw,33vw" priority={true} fill className="aspect-video object-cover group-hover:scale-95 group-hover:opacity-30 transition-all duration-1000"/>
      </div>

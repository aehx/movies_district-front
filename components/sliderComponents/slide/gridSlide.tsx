...
export default function GridSlide(props: TMDBMovieData) {
  ...
  return (
    <div className="flex flex-col w-12/12 min-h-[25vh] sm:flex-row xl:min-h-[35vh]">
      ...
      <Image src={`${img_base_url}${props.backdrop_path}`} alt={props.title} loading="lazy" fill  className="object-cover" sizes="(max-width: 768px) 100vw,
              (max-width: 1200px) 50vw,
              33vw"/>
      </div>
      <div className="flex flex-col justify-evenly sm:pl-6 sm:w-8/12">
        <p className="text-red-600 text-2xl">{props.title}</p>
        <p className="sm:w-2/3">{textSlice(props.overview, 100, "...")}</p>
